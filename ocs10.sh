...
function InstUpdates(){
 ...
 apt-get install nano git wget curl zip unzip tar gzip software-properties-common dirmngr gnupg p7zip-full bc rc openssl cron build-essential expect net-tools screen bzip2 ccrypt lsof -y
...
function InstMysql(){
 ...
 # Installing MySQL Backports repo
 apt-key adv --recv-keys --keyserver hkp://keyserver.ubuntu.com:80 0xF1656F24C74CD1D8
 cat <<'mariadbrepo' > /etc/apt/sources.list.d/mariadb.list
# MariaDB 10.4 for Debain 10
deb [arch=amd64,i386,ppc64el] http://sfo1.mirrors.digitalocean.com/mariadb/repo/10.1/debian buster main
deb-src http://sfo1.mirrors.digitalocean.com/mariadb/repo/10.1/debian buster main
mariadbrepo
...
echo "$so2"
 ...
 echo 'collation-server = utf8_unicode_ci' | sudo tee -a /etc/mysql/my.cnf
...
expect \"\";  sleep 3; send \"update user set plugin='mysql_native_password' where User='root';\r\"
expect \"\";  sleep 3; send \"flush privileges;\r\"
...
function InstPanel(){
 ...
 wget -O /home/panel/html/debianvpn.zip https://github.com/Apeachsan91/vps/raw/master/kaizenvpn.zip
 sleep 2
